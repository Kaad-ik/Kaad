...
public class UserService {
  ...
  private static final String USERNAME_DUPLICATED_MESSAGE = 
  ...
  private boolean isValid(User user) {
    return userRepository
        .findByUserNameAndPassword(user.getUserName(), user.getPassword()).isPresent();
  }
  ...
  void cook(Food food) {
    for (Ingredient ingredient : actualUser.getIngredients()) {
      if (food.getIngredients()
          .stream().anyMatch(i -> i.getType().equals(ingredient.getType()))) {
        int quantity = food.getIngredients()
            .stream().filter(ing -> ing.getType().equals(ingredient.getType()))
