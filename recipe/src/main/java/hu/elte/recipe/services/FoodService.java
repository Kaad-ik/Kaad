...
public class FoodService {
  ...
  public Food addFood(FoodHttpEntity entity) {
    try {
      Food food = new Food(entity.getName(), entity.getImgUrl(),
          validateIngredients(entity.getIngredients()), entity.getRecipe());
  ...
  private Predicate<? super Food> findItersectionIn(List<Ingredient> ingredients) {
    return food -> food.getIngredients()
        .stream().allMatch(haveTypeWithMoreOrEqualQuantity(ingredients));
  }
   ...
   */
  private Predicate<? super Ingredient> haveTypeWithMoreOrEqualQuantity(
  ...
  private List<Ingredient> validateIngredients(Collection<IngredientHttpEntity> entities) {
    ...
    if (result.stream().allMatch(ingredient -> ingredientTypeService
        .getByName(ingredient.getTypeName()).isPresent())) {
      return result;
    }
